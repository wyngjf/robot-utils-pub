"""
code taken from Manifold ODE, to be adapted
"""
import torch
from robot_utils.math.manifold.manifold import Manifold
from robot_utils.math.manifold.utils import EPS, sindiv, divsin
...
class Sphere(Manifold):
    ...
    def log(self, x: torch.Tensor, y: torch.Tensor):
        """
        maps a point y lying on the manifold into the tangent space of a point x on the manifold
        Args:
            x: (dim, ) or (batch, dim) basis point of the tangent space where y will be mapped
            y: (dim, ) or (batch, dim) the points on the manifold

        Returns: vectors in the tangent space of x
